...
interface RosterPlayer {
  player_id: string
  profiles: Pick<Profile, 'full_name' | 'nickname' | 'jersey_number'> | null
}
...
export default function AdminMatchGoals(): React.JSX.Element {
  ...
  const [gForm, setGForm] = useState<GoalFormState>({ scorer_id: '', assist_id: '', minute: '', is_own_goal: false, is_penalty: false, is_penalty_corner: false })
  const [cForm, setCForm] = useState<CardFormState>({ player_id: '', card_type: 'yellow', minute: '' })

  const { data, isLoading } = useQuery<MatchGoalsData>({
    queryKey: ['adminMatchGoals', id],
    queryFn: async (): Promise<MatchGoalsData> => {
      ...
      const matchData = matchRes.data as Match | null

      // Als er geen selectie is voor deze wedstrijd, val terug op alle teamleden
      let players = (rosterRes.data as unknown as RosterPlayer[]) || []
      if (players.length === 0 && matchData?.team_id) {
        const { data: members } = await supabase
          ...
          .eq('active', true)
        players = (members as unknown as RosterPlayer[]) || []
      }

      return {
        match: matchData,
        roster: players,
        goals: (goalsRes.data as unknown as GoalRecord[]) || [],
        cards: (cardsRes.data as unknown as CardRecord[]) || [],
      }
    },
    enabled: !!id,
  })
  ...
  const addGoalMutation = useMutation({
    mutationFn: async (values: GoalInsert): Promise<void> => { await supabase.from('goals').insert(values) },
    onSuccess: () => {
  ...
  const deleteGoalMutation = useMutation({
    mutationFn: async (goalId: string): Promise<void> => { await supabase.from('goals').delete().eq('id', goalId) },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminMatchGoals', id] })
    },
  })
  ...
  const addCardMutation = useMutation({
    mutationFn: async (values: CardInsert): Promise<void> => { await supabase.from('match_cards').insert(values) },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminMatchGoals', id] })
    },
  })
  ...
  const deleteCardMutation = useMutation({
    mutationFn: async (cardId: string): Promise<void> => { await supabase.from('match_cards').delete().eq('id', cardId) },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminMatchGoals', id] })
    },
  })

  async function addGoal(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    e.preventDefault()
  ...
  async function addCard(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    e.preventDefault()
